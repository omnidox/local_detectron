...
# First code block: Simple video display loop that shows the video feed in a window.
# It breaks and exits when the 'esc' key is pressed.      
    # for vis in tqdm.tqdm(demo.run_on_video2(cam)):
    #     cv2.namedWindow(WINDOW_NAME, cv2.WINDOW_NORMAL)
...
# It also breaks and exits when the 'esc' key is pressed. Also it will print the detected objects to the console.
...
# Third code block: Enhanced video display loop that adjusts the window size based on the frame's dimensions.
