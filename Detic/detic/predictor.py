...
class VisualizationDemo(object):
    ...
    def run_on_video2(self, video):
        ...
        Yields:
            tuple: A tuple containing the BGR visualizations of each video frame and the predictions.
        """
        video_visualizer = VideoVisualizer(self.metadata, self.instance_mode)
        ...
        if self.parallel:
            ...
            for cnt, frame in enumerate(frame_gen):
                ...
                if cnt >= buffer_size:
                    frame = frame_data.popleft()
                    vis_frame, predictions = process_predictions(frame, self.predictor.get())
                    yield vis_frame, predictions
            ...
            for frame in frame_gen:
                vis_frame, predictions = process_predictions(frame, self.predictor(frame))
                yield vis_frame, predictions
